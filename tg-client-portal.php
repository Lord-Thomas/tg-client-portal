 ...
 * Description: Espace admin sécurisé pour gérer Devis & Factures liés à des clients (avec mises à jour depuis GitHub).
 * Version: 0.1.2
 * Author: Thomas
...
define('TGCP_DIR', plugin_dir_path(__FILE__));
define('TGCP_URL', plugin_dir_url(__FILE__));

/* -------------------------------------------------------------------------- */
/*  Activation : rôles & capacités                                            */
/* -------------------------------------------------------------------------- */
...
register_activation_hook(__FILE__, function(){
  ...
  // S’assure que les CPTs existent avant flush
  tgcp_register_cpts();
...
add_action('admin_menu', function(){
  add_menu_page(
    ...
    'tgcp',
    function(){
      echo '<div class="wrap"><h1>Espace client</h1><p>Utilisez les sous-menus <strong>Devis</strong> et <strong>Factures</strong>.</p></div>';
    },
    'dashicons-portfolio',
    25
  );
...
function tgcp_users_select($label,$name,$selected){
  ...
  $html = '<p><label><strong>'.$label.'</strong><br><select name="'.$name.'" style="min-width:320px">';
...
add_action('save_post', function($post_id){
  ...
  $map = [
    ...
    '_tgcp_montant_ttc'    => function($v){ return ($v!=='' && $v!==null) ? round((float)$v,2) : ''; },
    '_tgcp_statut'         => function($v){ return $v ? sanitize_key($v) : ''; },
...
add_action('manage_devis_posts_custom_column', function($col,$post_id){
  if ($col==='tgcp_client'){
    ...
    echo ($m!=='' && $m!==null) ? esc_html(number_format((float)$m,2,',',' ')).' €' : '—';
...
/* -------------------------------------------------------------------------- */
/*  (Option) Mises à jour depuis GitHub via Plugin Update Checker             */
/* -------------------------------------------------------------------------- */

add_action('plugins_loaded', function(){
  $puc = TGCP_DIR . 'includes/vendor/plugin-update-checker/plugin-update-checker.php';
  if (file_exists($puc)) {
    require_once $puc;

    // Repo GitHub (public) – remplace l’URL si besoin.
    $tgcp_updater = Puc_v5_Factory::buildUpdateChecker(
      'https://github.com/Lord-Thomas/tg-client-portal/',
      __FILE__,
      'tg-client-portal'
    );

    // Suivre la branche "main" (ou bascule sur releases + tags plus tard).
    $tgcp_updater->setBranch('main');

    // Si repo privé : définir un token dans wp-config.php (define('TGCP_GITHUB_TOKEN','...'))
    if (defined('TGCP_GITHUB_TOKEN') && TGCP_GITHUB_TOKEN) {
      $tgcp_updater->setAuthentication(TGCP_GITHUB_TOKEN);
    }
  }
});
